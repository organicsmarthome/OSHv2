...
import osh.mgmt.localobserver.chp.eapart.CHPEAPartTemperatureControl;
 ...
 *
 */
public class ESHLThermalLocalObserver extends LocalObserver implements IHasState, IEventReceiver {
	private TreeMap<Long, AbstractCHPEAPart> encodingsmap = new TreeMap<Long, AbstractCHPEAPart>();
	 ...
	 * CONSTRUCTOR
	 */
	public ESHLThermalLocalObserver(IOSHOC controllerbox) {
		super(controllerbox);
	}
	...
	public void onDeviceStateUpdate() throws OCUnitException{
		if (this.getObserverDataObject() instanceof HeatingSystemData) {
			...
			if (oldState != chpex.isGasHeatingState() && expected != null && chpex.isGasHeatingState() != expected.getExpectedState()) {
...
//							0));
			...
			cpse.addPowerState(Commodity.REACTIVEPOWER, 0);
	...
	private void createNewEaPart(boolean currentState, long remainingRunningTime) {
		...
		if (model != null && heatingSystemData != null) {
			...
			ex = new CHPEAPartTemperatureControl(
					getDeviceID(), 
					getTimer().getUnixTime(), 
	...
	public void onNextTimePeriod() throws OSHException {
		...
		if (diff < 0 || diff > 1800) {
			if (lastExchange != null) {
				createNewEaPart(lastExchange.isGasHeatingState(), lastExchange.getMinRuntimeRemaining());
			} else {
				createNewEaPart(false, 0);
			}
		}
