 ...
 *
 */
public class StupidCHPEAPartTemperatureControl extends AbstractCHPEAPart {
	 ...
	 * CONSTRUCTOR
	 */
	public StupidCHPEAPartTemperatureControl(
			UUID deviceId, 
			long now,
			int electricalpower, 
			int thermalpower,
	...
	public Schedule getSchedule(BitSet solution) {
		...
		if (remainingRunningTime > 0) {
			...
			pr.setLoad(Commodity.WARMWATERPOWER, now, -thermalpower);
			laststate = true;
		} else {
			laststate = false;
		}
